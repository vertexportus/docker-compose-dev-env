...
class Git(base_command.BaseCommand):
    ...
    def argparse(parser, subparsers):
        ...
        parser_main.add_argument('project', help="project to run git command in ('all' for all projects, 'base' for base project, 'all+base' for all projects + base project)")
        parser_main.add_argument('params', nargs=argparse.REMAINDER, help='additional arguments passed directly into git command')
    ...
    def __git_clone(self, project, repo_url):
        ...
        try:
            repo = Repo.clone_from(f"{'https://github.com/' if env.git_use_https() else 'git@github.com:'}{repo_url}", f"src/{project}", progress=GitProgress())
